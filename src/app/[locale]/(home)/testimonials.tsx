		...
		<section className='relative py-14'>
			<div className='relative z-10 mx-auto max-w-screen-xl px-4 md:px-8'>
				<div className='max-w-xl sm:text-center md:mx-auto'>
					<h3 className='text-3xl font-semibold text-gray-800 sm:text-4xl'>Hear from our customers</h3>
				...
				<div className='mt-12'>
					<ul className='grid items-center gap-6 sm:grid-cols-2 lg:grid-cols-3'>
						{testimonials.map((item, idx) => (
							<li className='rounded-xl border bg-white shadow-md' key={idx}>
								<div className='p-4'>
									<svg
								...
								<figure>
									<blockquote>
										<p className='px-4 py-1 text-lg font-semibold text-gray-800'>{item.quote}</p>
									</blockquote>
									<div className='mt-6 flex items-center gap-x-4 bg-indigo-50 p-4'>
										<img className='h-16 w-16 rounded-full border-2 border-indigo-500' src={item.avatar} />
										<div>
											<span className='block font-semibold text-gray-800'>{item.name}</span>
											<span className='mt-0.5 block text-sm text-indigo-600'>{item.title}</span>
										</div>
									</div>
								</figure>
							</li>
