...
const AccordionTrigger = ({ children, className, ...props }: React.ComponentPropsWithoutRef<typeof Trigger>) => (
	<Header className='flex'>
		<Trigger
			className={twMerge(
				"relative flex flex-1 items-center justify-between rounded-md py-4 font-medium !outline-none !ring-0 transition-basic hover:text-shade-light [&[data-state=open]>svg]:rotate-180",
				className
			)}
			{...props}>
			{children}
