...
const SelectTrigger = ({ children, className, ...props }: React.ComponentPropsWithoutRef<typeof Trigger>) => (
	<Trigger
		className={twMerge(
			"w-ful flex h-10 items-center justify-between rounded-md border border-shade-light/40 border-slate-200 bg-white px-3 py-2 text-sm text-shade-light transition-basic placeholder:text-slate-500 hover:border-shade-light hover:bg-slate-50 hover:text-shade-light focus:outline-none focus:ring-2 focus:ring-shade-light focus:ring-offset-1 disabled:cursor-not-allowed disabled:opacity-50 dark:border-slate-800 dark:bg-slate-950 dark:ring-offset-slate-950 dark:placeholder:text-slate-400 dark:hover:bg-slate-800 dark:hover:text-slate-50 dark:focus:ring-slate-300 [&>span]:line-clamp-1",
			className
		)}
		{...props}>
		{children}
